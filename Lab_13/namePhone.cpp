...
int nameread(char *buffer, char *destChar);
...
int main()
{
    ...
    while (fgets(buffer, sizeof(buffer), stdin))
    {
        ...
        readField(buffer, index, AREA_CODE_SIZE, areacodePT);
        readField(buffer, index + 4, PREFIX_SIZE, prefixPT);
        readField(buffer, index + 8, LINE_NO_SIZE, linenoPT);

        cout << "Name: " << namePT << endl;
